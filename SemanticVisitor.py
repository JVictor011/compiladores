...
class NoTuple(Visitor):
    ...
    def visit(self, operator):
        evaluated_elements = [operator.registry(element.visit(operator)) for element in self.elements]
        return operator.success(TTuple(evaluated_elements).setMemory(operator))
